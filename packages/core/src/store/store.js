...
import { cloneDeep, isEqualWith, hasOwn } from '../utils/helper';
...
class Store extends BaseWatcher {
	constructor(combo, initialState = {}) {
		...
		merge(this.states, initialState);
	}
	...
	_makeRebuildData(source) {
		...
		let result = cloneDeep(source).map((it) => {
			let { data = {}, rebuilder = {} } = modulesMap.value[it.module] || {};
